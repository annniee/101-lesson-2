...
def detect_result(dealer_hand, player_hand)
  ...
  return :player_bust if bust?(player_hand)
  return :dealer_bust if bust?(dealer_hand)
...
loop do
  ...
  loop do
    ...
    dealer_score += 1 if winner == :dealer || winner == :player_bust
    player_score += 1 if winner == :player || winner == :dealer_bust
